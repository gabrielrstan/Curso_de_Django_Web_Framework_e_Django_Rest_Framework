...
class RecipeAPIv2TestMixin(RecipeMixin):
    ...
    def get_recipe_raw_data(self):
        return {
            'title': 'This is the title of the recipe',
            'description': 'This is the description of the recipe',
            'preparation_time': 1,
            'preparation_time_unit': 'Minutes',
            'servings': '1',
            'servings_unit': 'Person',
            'preparation_steps': 'This is the preparation steps of the recipe',
            'id': 1
        }
...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    def test_recipe_api_list_loads_recipes_by_category_id(self):
        ...
        api_ulr = reverse('recipes:recipes-api-list') + \
            f'?category_id={category_wanted.id}'  # type:ignore
        response = self.get_recipe_api_list(reverse_result=api_ulr)
    ...
    def test_recipe_api_list_logged_user_can_update_a_recipe(self):
        ...
        wanted_new_title = ('The new title updated by '
                            f'{author.username}')  # type:ignore
        # Action
        response = self.client.patch(
            reverse('recipes:recipes-api-detail',
        ...
        # Assertion
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.get('title'),  # type:ignore
                         wanted_new_title)

    def test_recipe_api_list_logged_user_cant_update_a_recipe_owned_by_another_user(self):  # noqa: E501
        # Arrange (config)
        recipe = self.make_recipe()
        access_data = self.get_auth_data(username='test_patch')
