...
class AuthorRegisterFormIntegrationTest(DjangoTestCase):
    ...
    def test_fields_cannot_be_empty(self, field, msg):
        ...
        # Esse retorna o HTML / content
        # self.assertIn(msg, response.content.decode('utf-8'))

        # Esse retorna mostra o a saida|esperado / context
        self.assertIn(msg, response.context['form'].errors.get(field))

    def test_username_field_min_length_should_be_4(self):
        self.form_data['username'] = 'Mia'
        url = reverse('authors:register_create')
        response = self.client.post(url, data=self.form_data, follow=True)
    ...
    def test_password_field_have_lower_upper_case_letter_and_numbers(self):
        ...
        msg = ('Password must have at least one uppercase letter, one '
               'lowercase letter and one number. The length should be at least'
               ' 8 characters.')
    ...
    def test_password_and_password_confirmation_are_equal(self):
        ...
        msg = 'Password and password2 must be the same'
        self.assertIn(msg, response.context['form'].errors.get('password'))
